...
def Phi():
    ...
    conv10a = Conv2D(1, 3, 1, activation = 'relu', padding='same')(conv1)
    conv10b = Conv2D(1, 3, 1, activation = 'relu', padding='same')(conv1)
    conv10c = Conv2D(1, 3, 1, activation = 'relu', padding='same')(conv1)
    conv10d = Conv2D(1, 3, 1, activation = 'relu', padding='same')(conv1)

    conv10 = concatenate([conv10a,conv10b,conv10c, conv10d],axis=-1)
...
#%% Reconstruction NW Psi, ground truth here is the original input image
    ...
    # conv1b = Conv2D(32, 3, 1, activation='linear', padding="same")(conv10a)
    ...
    # conv10d = Conv2D(1, 3, 1, activation = 'relu', padding='same')(conv4b)
    
    
    
    
    # conv1bb = Conv2D(32, 3, 1, activation='linear', padding="same")(conv10b)
    ...
    # conv10dd = Conv2D(1, 3, 1, activation = 'relu', padding='same')(conv4bb)
    
    
    # conv1bbb = Conv2D(32, 3, 1, activation='linear', padding="same")(conv10c)
    ...
    # conv10ddd = Conv2D(1, 3, 1, activation = 'relu', padding='same')(conv4bbb)
    # print(conv10ddd.shape)
    
    
   # recon = tf.keras.layers.add((conv10d,conv10dd,conv10ddd))
    recon2 = tf.keras.layers.add((conv10a,conv10b,conv10c))
    #conc = concatenate([recon,recon2],axis=-1)
    #recon = Conv2D(16, 3, 1, activation='relu', padding='same')(recon2)
    recon = Conv2D(32, 3, 1, activation='relu', padding='same')(recon2)

    #recon = Conv2D(256, 3, 1, activation='relu', padding='same')(recon2)
    recon = Conv2D(1, 3, 1, activation='relu', padding='same')(recon)
  ...
  #  out2 = [conv10b,conv10,conv10]
    ...
    model.compile(optimizer=SGD(lr=1e-5, momentum=0.99, decay=1e-2), loss=[Norm_Loss,coherence_penalty, "MSE"], loss_weights=[0.15,0.8,0.995], metrics=['accuracy'])
